            ...
            if file.endswith(".py"):
                ...
                with open(os.path.join(root, file), "r") as f:
                    ...
                    def document_data(i, name, line, other_docs, parent_string=""):
                        ...
                        new_docs = ""
                        new_docs += f"### [{name}](/{file_document_path}#{name.lower().replace(' ', '-').replace('.', '')}) ###\n\n"
                        new_docs += f"- [{class_declaration.strip()}](./../{file_path}#L{i + 1}) \n\n"
                        ...
                        other_docs += f"<details><summary>{name}</summary><br>{new_docs + new_documentation}</details>\n\n"
                        other_docs += "====================\n\n"
